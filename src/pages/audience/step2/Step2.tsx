...
const renderBlackout = (
  ...
  return (
    <div
      className="cursor-pointer bg-white text-main w-full text-base flex flex-col space-y-2"
    >
...
const AudienceStep2 = () => {
  ...
  return (
    <PageTemplate
      ...
      background='bg3'
      description={'For each poem, you will fill out a quick survey letting us know what you think of the work. Once you answer for one poem, you cannot return to it.'}
      duration={720}
      nextButton={{text: (poemIndex === blackoutPoems.length - 1) ? 'Finish' : 'Next Poem', action: handleNext}}
    >
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        {q.options!.map((option: string) => (
                            <RadioGroup.Item key={option} value={option}>
                                ...
                                <RadioGroup.ItemText className="text-sub">{option}</RadioGroup.ItemText>
                            </RadioGroup.Item>
                        ))}
                    </div>
                </RadioGroup.Root>
