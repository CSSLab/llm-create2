    ...
    <div className="w-full h-max flex flex-col space-y-6">
        <div className="w-full h-max flex flex-row justify-between">
          <div className="w-max flex flex-row space-x-2">
            <Button className={viewMode === 'edit' ? 'btn-small-inverted' : 'btn-small bg-grey'} onClick={() => setViewMode(viewMode === 'edit' ? 'blackout' : 'edit')}>{viewMode === 'edit' ? <RiEyeCloseLine /> : <MdOutlineMenuBook />} <p className="hidden md:block">{viewMode === 'edit' ? 'View as Blackout' : 'View as passage'}</p></Button>
            <Button className="btn-small-inverted" onClick={() => resetSelection()}><MdOutlineRestartAlt/> <p className="hidden md:block" >Reset poem</p></Button>
            <Button className="btn-small-inverted" onClick={() => copyPassage()}><MdContentCopy/></Button>
          </div>
                              
          <Button className="btn-small px-4" onClick={onSubmit}>Submit</Button>
        </div>
