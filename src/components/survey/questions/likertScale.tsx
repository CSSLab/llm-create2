  ...
  return (
    <div className="mb-4 space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-2">
        {question.labels && <span className="text-sub hidden md:block">{question.labels.min}</span>}
        <RadioGroup.Root
        ...
        >
          {range.map((num) => (
            ...
            >
              ...
              <RadioGroup.ItemText className="text-sub font-light md:hidden">{(num === question.scaleMin ? question.labels!.min : num === question.scaleMax ? question.labels!.max : "")}</RadioGroup.ItemText>
            </RadioGroup.Item>
          ))}
        </RadioGroup.Root>
        {question.labels && <span className="text-sub hidden md:block">{question.labels.max}</span>}
      </div>
    </div>
  );
};
